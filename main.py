...
def obtener_historial(id_num):
    """Recupera contexto para la memoria del bot"""
...
# --- 🏦 TASAS MULTI-FUENTE CON RESPALDO ---
def obtener_tasas_dinamicas():
    ...
    for url in fuentes:
        try:
            ...
            if 'promedio' in str(res):
                tasa_usd = float(res.get('promedio') or res['monedas']['usd']['price'])
            elif 'rates' in res:
                tasa_usd = float(res['rates']['VES'])
            
            if tasa_usd and tasa_usd > 300: # Validación para 2026
                cur.execute("UPDATE config SET value = %s WHERE key = 'last_tasa'", (str(tasa_usd),))
                conn.commit()
                break
        except: continue

    if not tasa_usd:
        cur.execute("SELECT value FROM config WHERE key = 'last_tasa'")
        res_db = cur.fetchone()
        tasa_usd = float(res_db['value']) if res_db else 385.50
    
    cur.close(); conn.close()
    return {"usd": tasa_usd, "eur": tasa_usd * 1.08}
...
def webhook():
    ...
    if 'messages' in data:
        ...
        if numero == ADMIN_PHONE:
            intent = consultar_ia("Tags: [APROBAR:REF], [LISTAR_PENDIENTES], [CONSULTAR_TASA], [BOT:ON], [BOT:OFF].", msg.get('text', {}).get('body', ""))
            if "[CONSULTAR_TASA]" in intent:
                t = obtener_tasas_dinamicas()
                enviar_meta(ADMIN_PHONE, f"📊 *Tasas BCV:* USD: {t['usd']} | EUR: {t['eur']}")
        ...
        if msg.get('type') == 'location':
            ...
            enviar_meta(numero, "📍 Ubicación guardada. ¡Gracias!"); return jsonify({"status": "ok"}), 200

        tasas = obtener_tasas_dinamicas()
        historial = obtener_historial(id_num)
        instruccion = f"{BUSINESS_CONTEXT}\nTasa: {tasas['usd']} Bs.\nREGLAS: [AGENDAR] al iniciar. Al terminar: Resume, da total ($ y Bs), pide GPS y usa [FINALIZAR]. [CANCELAR] si desiste."
        
        texto_cli = msg.get('text', {}).get('body', "Archivo")
        res_ia = consultar_ia(instruccion, texto_cli, historial, msg.get('image', {}).get('id') if msg.get('type') == 'image' else None)

        if "[AGENDAR]" in res_ia: ejecutar_logica_db(id_num, "AGENDAR")
        if "[FINALIZAR]" in res_ia: ejecutar_logica_db(id_num, "FINALIZAR")
        if "[CANCELAR]" in res_ia: ejecutar_logica_db(id_num, "CANCELAR")
